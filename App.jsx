        ...
        {allTests.length===0?<div className="empty">Inga tester tillgängliga.</div>
          :allTests.map(t=>(
            <div key={t.id} className="row">
              ...
              <button className="btn btn-p btn-sm" onClick={()=>onSelect(t)}>Starta</button>
            </div>
          ))
        }
      </div>
...
const GROUP_INFO={
  ...
  "1v":{label:"Grupp 1",desc:"–ar, –ade, –at  (måla → målade)"},
  "2a":{label:"Grupp 2a",desc:"–er, –de, –t  (bygga → byggde, köra → körde)"},
  "2b":{label:"Grupp 2b",desc:"–er, –te, –t  (p/t/k/s/x i stammen: söka → sökte)"},
  "3v":{label:"Grupp 3",desc:"lång vokal, –r, –dde, –tt  (tro → trodde, bo → bodde)"},
  "4v":{label:"Grupp 4",desc:"stark (avljud) eller oregelbunden  (skriva → skrev)"},
};
...
function Quiz({test,onFinish,onBack}){
  ...
  if(phase==="group"&&isTwoStage){
    ...
    // Map display key → actual correctGroup value stored in question data
      ...
      <div>
        ...
        <p style={{fontSize:14,color:"var(--color-text-secondary)",marginBottom:".75rem"}}>
          {q.groupType==="adj_abc"
            ...
            :q.groupType==="comp_1234"
            ?"Vilken komparationsgrupp (1–4) tillhör adjektivet?"
            :"Vilken verbgrupp (1, 2a, 2b, 3 eller 4) tillhör verbet?"}
        </p>
...
export default function App(){
  ...
  useEffect(()=>{
    (async()=>{
      const c=await load("quiz_custom"); const r=await load("quiz_results");
  ...
  async function saveTest(t){
    ...
    setCustom(u); await save("quiz_custom",u);
  }
  async function deleteTest(id){
    const u=custom.filter(t=>t.id!==id); setCustom(u); await save("quiz_custom",u);
  }
  async function handleFinish(score,total,answers){
    ...
    const u=[...results,r]; setResults(u); await save("quiz_results",u);
  }
